...
export default function RepositoryListItem({ repo }) {
  ...
  return (
      <div className="repositoryListItem-wrapper">
          <span className="repositoryListItem-name"><a href={repo.link} rel="noopener noreferrer" target='_blank'>{repo.name}</a></span>
          <p className="repositoryListItem-description">{repo.description}</p>
          <div className="repositoryListItem-data">{starCount}{forkCount}</div>
      </div>
  )
}
